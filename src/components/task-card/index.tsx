...
const TaskCard = ({ task }: { task: Task }) => {
  ...
  const Backdrop = () => (
    <motion.div
      className="fixed inset-0 bg-black/05 backdrop-blur-sm z-10"
  ...
  return (
    <>
      <AnimatePresence>
        {currentCard && <Backdrop />}
      </AnimatePresence>
      <AnimatePresence>
        <motion.div
          className={`cursor-pointer overflow-hidden h-fit ${currentCard ? 'z-20 fixed inset-0 mx-auto mt-16' : 'cursor-grab active:cursor-grabbing'}`}
          draggable={!currentCard}
          style={currentCard ? { maxWidth: '30%', minWidth: '350px', maxHeight: '70%' } : {}}
          onClick={() => setCurrentCard(task.id)}
          initial={{ scale: 1 }}
          animate={{ scale: currentCard ? 1.1 : 1 }}
          layout
          transition={{ duration: 0.3 }}
        >
          {/* <Card className='rounded-[18px] overflow-hidden duration-300 shadow-none'> */}
